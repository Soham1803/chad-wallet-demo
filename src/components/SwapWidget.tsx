...
export default function SwapWidget({
  ...
  return (
    <div className="bg-transparent p-4 flex flex-col gap-4 relative">
      ...
      {showSettings && (
        <div className="bg-dark-card border border-dark-border rounded-xl p-3 text-xs space-y-3">
          ...
          <div className="text-[10px] text-foreground/40">
            Higher slippage protects swaps from failing during volatile price
            fluctuations but may result in less tokens received.
          </div>
        </div>
      )}
      ...
      <div className="bg-dark-card border border-dark-border/60 rounded-xl p-3.5 space-y-2">
        ...
        <div className="relative flex w-full items-center gap-3">
          <input
            ...
            disabled={loading}
            className="flex-1 bg-transparent w-1/2 text-lg font-bold font-mono text-foreground/95 focus:outline-none placeholder:text-foreground/20"
          />
      ...
      {numFrom > 0 && (
        <div className="bg-dark-card/30 border border-dark-border/40 rounded-xl p-3 text-[10px] text-foreground/50 font-mono space-y-1.5">
          <div className="flex justify-between">
            <span>Rate</span>
            <span>
              1 SOL = {rate < 1 ? rate.toFixed(6) : rate.toFixed(2)}{" "}
              {token.symbol}
            </span>
          </div>
          <div className="flex justify-between">
            ...
            <span
              className={
                numFrom > 50 ? "text-yellow-500 font-bold" : "text-brand-green"
              }
            >
              {priceImpact}
            </span>
      ...
      {!ready ? (
        ...
        <button
          onClick={login}
          className="w-full h-12 rounded-xl text-white font-bold text-sm shadow-lg hover:shadow-brand-green/20 hover:scale-[1.01] active:scale-[0.99] transition-all flex items-center justify-center gap-2 cursor-pointer"
        >
